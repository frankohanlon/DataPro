...
May 2010 work to add CR1000 format compatibility
...
## tmstmpcol = regular date & time format (but may need reformatting)                                                                                ##
## num       = normal float                                                                                                                          ##
## therm     = thermistor... specify the coefficients in the coefficient table
## therm1    = thermsistor with a look up table specified in the main config file #1.
## therm2    = thermistor with a look up table specified in the main config file #2.                                                                      ##
...
for element in siteList :
    ...
    if d_type != 'ignore' and d_type != 'datey' and d_type != 'dated' and d_type != 'dateh' and d_type != 'tmstmpcol' :
        ...
        if os.path.isfile(out_file_name) :            
            fileinfo = os.stat(out_file_name)
            try :
                output_file[ col_type['d_element']] = open( out_file_name, 'a+')
                data_linez = output_file[ col_type['d_element']].readlines()
                last_line = data_linez[-1]
